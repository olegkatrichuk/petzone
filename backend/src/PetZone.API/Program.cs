...
try { await PetZone.Volunteers.Infrastructure.VolunteersSeeder.SeedAsync(app.Services); }
catch (Exception ex) { app.Logger.LogError(ex, "VolunteersSeeder failed: {Message}", ex.InnerException?.Message ?? ex.Message); }
